...
# Install required PHP extensions, tools, and wp-cli
...
# Embed wp-cli.config.php content directly
RUN cat > wp-config.php <<EOL
<?php
/**
 * Used by wp-cli during Docker build on Railway.app - EMBEDDED IN DOCKERFILE
 */
return array(
...
# Install WordPress core, create admin user, set URLs, install and activate plugin - IMPROVED WP-CLI INSTALL COMMAND
RUN wp core install --path='/var/www/html' --url='https://${RAILWAY_STATIC_URL}' --title='Llama3 Chatbot Site' --admin_user='admin' --admin_password='password' --admin_email='admin@example.com' --skip-email --allow-root ; then \
 ...
 * Description: Professional chatbot powered by Llama 3, WordPress plugin with advanced features.
 * Version:     1.3.0
 * Author:      Your Name
...
if (!file_exists(\$autoload_path)) {
    add_action('admin_notices', function() use (\$plugin_dir) {
        ...
        <div class="error notice is-dismissible">
            <p><?php _e( 'Llama 3 Chatbot: <strong>Dependencies not installed.</strong>', 'llama3-chatbot' ); ?></p>
            <p><?php _e( 'Install dependencies using Composer:', 'llama3-chatbot' ); ?></p>
            <ol>
                <li><?php _e( 'Access server via SSH/terminal.', 'llama3-chatbot' ); ?></li>
                <li><?php printf( __( 'Navigate to plugin dir: <code>%s</code>', 'llama3-chatbot' ), esc_html( \$plugin_dir ) ); ?></li>
                <li><?php _e( 'Run: <code>composer install</code>', 'llama3-chatbot' ); ?></li>
                <li><?php _e( 'Reload page after Composer finishes.', 'llama3-chatbot' ); ?></li>
            </ol>
            <button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e( 'Dismiss notice.', 'llama3-chatbot' ); ?></span></button>
        </div>
        <?php
    });
    return;
...
try {
    ...
    error_log("Llama 3 Chatbot: Model/pipeline loaded in " . gethostname());
} catch (\Exception \$e) {
    add_action('admin_notices', function() use (\$e) {
        ?>
        <div class="error notice is-dismissible">
            <p><?php _e( 'Llama 3 Chatbot: Error loading model. Check config/logs.', 'llama3-chatbot' ); ?></p>
            <button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e( 'Dismiss notice.', 'llama3-chatbot' ); ?></span></button>
        </div>
        <?php
    });
    error_log('Llama 3 Chatbot: Model load error: ' . \$e->getMessage());
    return;
}
...
function generarImagenDesdeAPI_llama3_chatbot(string \$prompt): ?string {
    ...
    if (empty(\$apiKey)) {
        error_log('Llama 3 Chatbot: HF API Key missing for images.');
        return null;
    }
    ...
    if (curl_errno(\$ch)) {
        error_log('Llama 3 Chatbot: cURL error for image: ' . curl_error(\$ch));
        curl_close(\$ch);
        return null;
    }
    curl_close(\$ch);
    ...
    if (\$httpCode === 200) {
        ...
        error_log("Llama 3 Chatbot: Image API error. Code: " . \$httpCode . ", Response: " . \$response);
        return null;
    }
}
...
function generarMusicaDesdeAPI_llama3_chatbot(string \$prompt): ?string {
    ...
    \$apiKey = esc_attr(get_option('llama3_chatbot_hf_api_key'));
     ...
     if (empty(\$apiKey)) {
        error_log('Llama 3 Chatbot: HF API Key missing for music.');
        return null;
    }
    ...
    if (curl_errno(\$ch)) {
        error_log('Llama 3 Chatbot: cURL error music: ' . curl_error(\$ch));
        curl_close(\$ch);
        return null;
    }
    curl_close(\$ch);
    ...
    if (\$httpCode === 200) {
        ...
        error_log("Llama 3 Chatbot: Music API error. Code: " . \$httpCode . ", Response: " . \$response);
        return null;
    }
}
...
function inferirTextoPipelineOptimo_llama3_chatbot(string \$prompt, TextGenerationPipeline \$pipeline, array \$generationConfig = [], array &\$conversationHistory): array|false {
    ...
    try {
        ...
        error_log('Llama 3 Chatbot: Text pipeline error: ' . \$e->getMessage());
        return false;
    }
}
...
function api_settings_section_callback() {
    _e('Configure API settings for external services.', 'llama3-chatbot');
}
function chatbot_settings_section_callback() {
    _e('Customize chatbot display and behavior.', 'llama3-chatbot');
}
function generation_settings_section_callback() {
    _e('Adjust text generation parameters, including streaming chunk size.', 'llama3-chatbot');
}
...
function split_tokens_field_callback() {
    ...
    echo "<input type='number' name='llama3_chatbot_split_tokens' value='{\$splitTokens}' class='small-text'> <p class='description'>". __('Response chunk size (words) for streaming.', 'llama3-chatbot') ."</p>";
}
?>
<style>
...
# Remove temporary plugin copy directory
RUN rm -rf /var/www/html/llama3-chatbot-pro

# Copy plugin again after wordpress install (for correct file permissions)
